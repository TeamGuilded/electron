...
### `systemPreferences.getUserDefaultForSuite(suite, key, type)` _macOS_
...
Returns `any` - The value of `key` in `NSUserDefaults` for the given `suite`.
...
### `systemPreferences.setUserDefaultForSuite(suite, key, type, value)` _macOS_
...
Set the value of `key` in `NSUserDefaults` for the given `suite`.
...
### `systemPreferences.removeUserDefaultForSuite(suite, key)` _macOS_

* `suite` String
* `key` String

Removes the `key` in `NSUserDefaults` for the given `suite`.
