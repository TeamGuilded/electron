...
WebContents.prototype._init = function () {
  ...
  // Handle context menu action request for spelling suggestions
  this.on('context-menu', function (event, params, callback) {
    // Access Menu via electron.Menu to prevent circular require.
    const menu = new electron.Menu()
    for (const suggestion in params.dictionarySuggestions) { menu.append(new electron.MenuItem({ label: params.dictionarySuggestions[suggestion] })) }
    menu.popup({
      window: event.sender.getOwnerBrowserWindow(),
      x: params.x,
      y: params.y,
