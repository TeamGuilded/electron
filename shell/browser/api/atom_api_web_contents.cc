...
class BrowserSideMessagePort : public gin::Wrappable<BrowserSideMessagePort> {
 public:
  ...
  static gin::Handle<BrowserSideMessagePort> Create(v8::Isolate* isolate, blink::MessagePortChannel channel) {
  ...
  gin::ObjectTemplateBuilder GetObjectTemplateBuilder(
    ...
    return gin::Wrappable<BrowserSideMessagePort>::GetObjectTemplateBuilder(isolate);
  }

  static gin::WrapperInfo kWrapperInfo;

  blink::MessagePortChannel ReleaseChannel() {
    return std::move(channel_);
  }

 private:
  explicit BrowserSideMessagePort(blink::MessagePortChannel channel): channel_(channel) {}

  blink::MessagePortChannel channel_;
};
gin::WrapperInfo BrowserSideMessagePort::kWrapperInfo = {gin::kEmbedderNativeGin};
...
void WebContents::PostMessage(blink::TransferableMessage message) {
  ...
  for (auto& port : message.ports) {
    wrapped_ports.emplace_back(BrowserSideMessagePort::Create(isolate(), std::move(port)));
  }
  EmitWithSender("-ipc-ports", bindings_.dispatch_context(), base::nullopt, false, "foo", std::move(wrapped_ports));
}
...
void WebContents::PostIPCMessage(v8::Local<v8::Value> message) {
  ...
  if (!gin::ConvertFromV8(isolate(), message, &wrapped_ports)) {
    isolate()->ThrowException(v8::Exception::Error(
        gin::StringToV8(isolate(), "not ports")));
    return;
  }
  for (auto& wrapped_port : wrapped_ports) {
    transferable_message.ports.emplace_back(std::move(wrapped_port->ReleaseChannel()));
  }
  ...
  electron_ptr->PostMessage(std::move(transferable_message));
}
